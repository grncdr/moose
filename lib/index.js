...
var Moose = comb.singleton(migration, {
			instance : {
				...
				createConnection : function(options) {
					...
					if (!this.conns[db]) this.conns[db] = {ready: false};
					if (!this.conns[db].ready) {
						var conn = this.conns[db];
						conn.options = options;
						conn.type = options.type || "mysql";
						var adapter = this.adapter;
						if (adapter) {
							conn.client = new adapter.client(options);
						} else {
						...
						this.models[db] = {};
						conn.ready = true;
						if (!this.defaultDatabase) this.defaultDatabase = db;
					}
				},
				...
				closeConnection : function() {
					var ret;
					for (var db in this.conns) {
						if (this.conns[db].ready) {
							ret = this.conns[db].client.close();
							this.conns[db].ready = false;
						} else {
							ret = new Promise();
							ret.callback();
						}
				...
				getConnection : function(autoClose, database) {
					...
					return this.conns[database].client.getConnection(autoClose)
				},
				...
				transaction : function(database) {
					return this.conns[database].client.transaction(database);
				},
				...
				loadSchema : function(tableName, database) {
					var promise = new Promise();
					var db = database || this.defaultDatabase;
					if (this.conns[db].ready) {
						var schema;
				 ...
				 *
				 */
				addModel : function(table, db, options) {
					var args = [];
					...
					if (comb.isObject(db)) {
						options = db;
						db = (table instanceof Table && table.database)
							? table.db
					...
					if (table instanceof Table) {
						var models;
						if (!(models = this.models[db])) {
							models = this.models[db] = {};
						}
						m = models[table.tableName] = model.create(table, this, options);
						return m;
					} else if (typeof table !== "string") {
						throw new Error('Table must be a Table object or string');
					} else {
						if (this.conns[db].ready) {
							options = options || {};
							var models;
							...
							if (models[table]) {
								...
								var schemas = this.schemas[db];
								if (!schemas || !table in this.schemas[db]) {
									this.loadSchema(table, db).then(
											function(schema) {
				...
				getters : {
					...
					database : function(){
						var db = this.defaultDatabase;
						return this.conns[db].ready ? db : null;
					}
				},
			}
		});
